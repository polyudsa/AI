...
class tspSolutionByGeneticAlgorithm():
    ...
    def __init__(self, num_city, population_size=50, iteration=100, start_city=1, end_city=1, data=None):
        ...
        self.population = self.inital_population(self.start_city, self.end_city, self.distance_cities, population_size,
                                                 num_city)
    ...
    def random_init(self, population_size, num_city):
    ...
    def inital_population(self, start_city, end_city, distance_cities, population_size, num_city):
        ...
        for i in range(population_size):
            ...
            if i == 0:
                ...
                while len(rest) != 0:
                    ...
                    rest.remove(tmp_choose)
            # add end city index
            result_best_path_one.append(end_index)
    ...
    def genetric_cross(self, parent_x, parent_y):
        ...
        path_list.remove(self.start_city)
        path_list.remove(self.end_city)
        order_list = list(random.sample(path_list[2:len(parent_x)-2], 2))
        path_list.append(self.start_city)
        path_list.append(self.end_city)
        order_list.sort()
    ...
    def genetric_mutate(self, gene):
        ...
        path_list.remove(0)
        path_list.remove(len(gene)-1)
        order_list = list(random.sample(path_list, 2))
        path_list.append(0)
        path_list.append(len(gene))
...
if __name__ == '__main__':
    # 10 cities
    data = read_data.read_tsp('data/ch10.tsp')
    data = np.array(data)
    data = data[:, 1:]
    mode = tspSolutionByGeneticAlgorithm(num_city=data.shape[0], population_size=POPULATION_SIZE,
                                         iteration=MAX_ITERATION, start_city=5, end_city=8, data=data.copy())
    mode.run()
    # 20 cities
